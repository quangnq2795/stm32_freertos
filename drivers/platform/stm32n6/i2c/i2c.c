...
static int i2c_hw_init(const i2c_desc_t *d, I2C_HandleTypeDef *hi2c);
...
void HAL_I2C_MspDeInit(I2C_HandleTypeDef *hi2c)
{
  ...
  HAL_GPIO_DeInit(d->scl_port, d->scl_pin);
  HAL_GPIO_DeInit(d->sda_port, d->sda_pin);
  clk_disable_i2c(d->instance);
}
...
static int i2c_hw_init(const i2c_desc_t *d, I2C_HandleTypeDef *hi2c)
{
  ...
  if (clk_config_i2c_source(d->instance) != 0) {
    return -1;
  }
  clk_enable_gpio_port(d->scl_port);
  if (d->sda_port != d->scl_port) {
    clk_enable_gpio_port(d->sda_port);
  }
  clk_enable_i2c(d->instance);
  ...
  gpio.Pin = d->scl_pin;
  gpio.Alternate = d->scl_af;
  HAL_GPIO_Init(d->scl_port, &gpio);

  gpio.Pin = d->sda_pin;
  gpio.Alternate = d->sda_af;
  HAL_GPIO_Init(d->sda_port, &gpio);

  hi2c->Instance = d->instance;
  hi2c->Init.Timing = d->clock_speed;
