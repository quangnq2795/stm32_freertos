...
static TimerHandle_t s_led_timer[LED_COUNT];
static char s_led_timer_name[LED_COUNT][4];

static void hmi_led_timer_cb(TimerHandle_t t)
{
  uintptr_t const id = (uintptr_t)pvTimerGetTimerID(t);
  if (id < (uintptr_t)LED_COUNT) {
...
{
  if (id >= LED_COUNT || s_led_timer[id] != NULL) {
    return;
  }
...
{
  for (led_id_t i = 0U; i < LED_COUNT; ++i) {
